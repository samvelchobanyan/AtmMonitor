...
export function startRouter() {
    ...
    page("/incassate", async (ctx) => {
        ...
        mountComponent("incassate-analythics", "Անալիտիկա | Ինկասացիա", ctx.query, "/incassate");
    });
    ...
    page("/all-atms", async (ctx) => {
        ...
        mountComponent("all-atms-page", "Բանկոմատների ցուցակ", ctx.query, "/all-atms");
    });
    ...
    page("/edit-atm/:id", async (ctx) => {
        if (!customElements.get("edit-atm-page")) {
            await import("../pages/edit-atm.js");
        }
        mountComponent(
            "edit-atm-page",
            "Խմբագրել բանկոմատ",
            { id: ctx.params.id },
            `/edit-atm/${ctx.params.id}`,
        );
    });
