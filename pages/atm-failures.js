...
class AtmFailures extends DynamicElement {
  ...
  async fetchTopSummary() {
    ...
    try {
      const response = await this.fetchData(
        `/device-faults/summary?${queryString}`
      );
      this.setState({
  ...
  async fetchChartSummary() {
    ...
    try {
      const chartResponse = await this.fetchData(
        `/device-faults/summary?${queryString}`
      );
  ...
  buildQueryString(startDate = null, endDate = null) {
    ...
    if (this.tableActiveTab)
      queryString.append('deviceId', this.tableActiveTab);
  ...
  template() {
    ...
    // todo tell Aram that there is no deice with 12 id in api
