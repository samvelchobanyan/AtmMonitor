...
class AtmDetails extends DynamicElement {
  ...
  _transformToTransactionDynamics(data) {
    ...
    const transactionDynamics = exchange_dynamic.hourly_data.map(
      (exchangeItem) => {
        const hour = exchangeItem.hour;

        // Find corresponding to dispense data for this hour
        const dispenseItem = dispense_dynamic.hourly_data.find(
          (item) => item.hour === hour
        );
        ...
        const depositItem = deposit_dynamic.hourly_data.find(
          (item) => item.hour === hour
        );
  ...
  template() {
    ...
    const transactionDynamics = encode(
      this._transformToTransactionDynamics(
        data.transactions_summary.transaction_dynamics
      )
    );
                        ...
                        <div class="column sm-6">
                            <div class="infos infos_margin">
                                <info-card
                                    ...
                                    value="${
                                      data.balance_info.last_encashment_amount
                                    }"
                                    value-currency="֏"
                ...
                <div class="container">
                    ...
                    <div class="infos infos_margin">
                       <info-card
                            title="Աշխատաժամանակ"
                            value="${
                              atmWorkHours.work_hours_per_day[0].working_percent
                            }%"
                        ...
                        <info-card
                            title="Խափանում"
                            value="${
                              atmWorkHours.work_hours_per_day[0]
                                .non_working_percent
                            }%"
                            icon="icon icon-clock"
                            show-border="true"></info-card>
