...
import {
  formatDateOnly,
  formatTimeOnly,
} from '../../core/utils/date-transformer.js';
...
class ListView extends DynamicElement {
  ...
  template() {
    ...
    if (loading) return `<div class="loading">Տվյալները բեռնվում են…</div>`;
    if (error) return `<div class="error">Տվյալների բերնման սխալ</div>`;
    if (!rawTemplate)
      return `<div class="error">No template provided inside list-view.</div>`;
