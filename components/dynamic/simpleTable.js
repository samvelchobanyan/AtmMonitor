...
export class SimpleTable extends DynamicElement {
  ...
  onAfterRender() {
    ...
    const searchable =
      this.getAttribute('searchable') === 'false' ? false : true;
    if (this.$('table')) {
      loadDataTableModule().then((sdtLib) => {
        ...
        this.datatableInstance.on('datatable.sort', () =>
          this.addEventListeners()
        );
        this.datatableInstance.on('datatable.search', () =>
          this.addEventListeners()
        );
  ...
  addEventListeners() {
    ...
    if (exportBtn) {
      this.addListener(exportBtn, 'click', () => {
        const filenameBase = (
          this.getAttr('export-filename') || 'export'
        ).replace(/\.csv$/i, '');
      ...
      colIndices.forEach((i) => {
        ...
        this.addListener(cell, 'click', (event) => {
          ...
          const rowIndex = parseInt(
            clickedRow.getAttribute('data-row-index'),
            10
          );
